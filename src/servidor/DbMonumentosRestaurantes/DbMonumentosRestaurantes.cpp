...
*/
DbMonumentosRestaurantes::DbMonumentosRestaurantes(int MAX_NUM_MONUMENTOS_DEVOLVER,int MAX_NUM_RESTAURANTES_DEVOLVER) {
    this->dbMonumentos= new SuffixTree<Monumento*>(MAX_NUM_MONUMENTOS_DEVOLVER);
    this->MAX_NUM_MONUMENTOS_DEVOLVER=MAX_NUM_MONUMENTOS_DEVOLVER;
    this->MAX_NUM_RESTAURANTES_DEVOLVER=MAX_NUM_RESTAURANTES_DEVOLVER;
...
*/
double DbMonumentosRestaurantes::calcular_distancia(Restaurante& a , Monumento& b){
...
*/
void DbMonumentosRestaurantes::buscarRestaurante(Monumento monumento_seleccionado, Lista<Restaurante>& resultados){
  resultados.clear();

    Restaurante seleccionados[MAX_NUM_RESTAURANTES_DEVOLVER];
  int almacenados=0;
  ...
  if(dbRestaurantes.size()>0){
    ...
    while (dbRestaurantes.next(rest_actual)) {
      if (almacenados<MAX_NUM_RESTAURANTES_DEVOLVER) {//hay menos de MAX_NUM_RESTAURANTES_DEVOLVER seleccionados
        ...
        distancia_seleccionados[almacenados]=calcular_distancia(rest_actual,monumento_seleccionado);
        ++almacenados;
      }else{
        ...
        double distancia_actual=calcular_distancia(rest_actual,monumento_seleccionado);
