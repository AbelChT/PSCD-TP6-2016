...
// Tratamiento de los monumentos
    ...
    for (int i = 0; i <
                    json_monumentos["features"].size(); ++i) {//itero hasta haber insertado todos los monumentos a la estructura
        ...
        double UTM_north = (*it_monumentos).at("geometry").at(
                "coordinates")[0].get<double>();//coordenadas del monumento
...
// Tratamiento de los restaurantes
    ...
    for (int i = 0; i <
        ...
        double UTM_east = (*it_restaurantes).at("geometry").at(
                "coordinates")[1].get<double>();//coordenadas del monumento
