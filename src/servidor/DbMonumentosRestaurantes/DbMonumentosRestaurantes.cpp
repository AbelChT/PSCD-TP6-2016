...
bool DbMonumentosRestaurantes::buscarRestaurante(double monumento_UTMNorthing, double monumento_UTMEasting,
    ...
    if (this->dbRestaurantes.size() > 0) {
        this->mtx_buscarRestaurante.lock();//cojo el mutex

        //creo un restaurante con un valor aleatorio
        Restaurante *rest_actual;

        //Pongo el primer elemento como seleccionado
        this->dbRestaurantes.begin();
        this->dbRestaurantes.next(rest_actual);
        ...
        this->mtx_buscarRestaurante.unlock();//suelto el mutex
        return true;
    }
    else return false;
}
